...
export const MOCK_PROFILES: MockProviderProfile[] = [
  {
    ...
    type: "MOCK",
    supportedActions: ["DISCOVER", "DESCRIBE", "RESERVE", "ACTIVATE", "DEACTIVATE", "MEASURE", "RELEASE", "RECONCILE", "SETTLE"],
  ...
  },
  {
    ...
    type: "MOCK",
    supportedActions: ["DISCOVER", "DESCRIBE", "RESERVE", "ACTIVATE", "DEACTIVATE", "MEASURE", "RELEASE", "RECONCILE", "SETTLE", "RENEW", "SUSPEND", "RESUME"],
  ...
  },
  {
    ...
    type: "MOCK",
    supportedActions: ["DISCOVER", "DESCRIBE", "RESERVE", "ACTIVATE", "DEACTIVATE", "MEASURE", "RELEASE", "RECONCILE", "SETTLE", "SWITCH", "RENEW"],
...
export async function executeMockAction(
  ...
  action: ActionType,
  opts: { providerResourceId: string; idempotencyKey: string; faultMode?: FaultMode; signal?: AbortSignal; deadline?: Date }
): Promise<AdapterActionResult> {
  const key = opts.idempotencyKey;
  const fault = opts.faultMode ?? "SUCCESS";
  const rid = opts.providerResourceId;

  // Observational actions (DISCOVER, DESCRIBE, MEASURE, RECONCILE) do NOT
  // create resource state — they query it. Only mutating actions create state.
  const isObservational = action === "DISCOVER" || action === "DESCRIBE" || action === "MEASURE" || action === "RECONCILE";
  let resourceState: any;
  if (!isObservational) {
    resourceState = await getOrCreateResourceState(profile.code, rid);
  } else {
    // Query existing state without creating.
    resourceState = await db.providerResourceState.findUnique({
      where: { providerCode_resourceIdentifier: { providerCode: profile.code, resourceIdentifier: rid } },
    });
    if (!resourceState) {
      // No state exists — return a result for an unknown resource.
      if (action === "DISCOVER") {
        return { ok: true, providerResourceId: rid, state: "unknown", capabilities: [{ type: profile.capabilityType, advertised: profile.advertised }], idempotent: true };
      }
      if (action === "DESCRIBE") {
        return { ok: true, providerResourceId: rid, state: "unknown", description: { providerCode: profile.code, capabilityType: profile.capabilityType, advertised: profile.advertised, coverage: profile.coverage }, idempotent: true };
      }
      if (action === "MEASURE") {
        return { ok: true, providerResourceId: rid, state: "unknown", measurement: observedMeasurement(profile, key), idempotent: true };
      }
      if (action === "RECONCILE") {
        return { ok: true, providerResourceId: rid, state: "unknown", idempotent: true, reconciled: true };
      }
    }
  }
  ...
  // DESCRIBE: return generic resource metadata (no state change).
  if (action === "DESCRIBE") {
    return {
      ok: true, providerResourceId: rid,
      state: resourceState.state,
      description: {
        providerCode: profile.code,
        capabilityType: profile.capabilityType,
        advertised: profile.advertised,
  ...
  // DISCOVER: return advertised capabilities (no state change).
  if (action === "DISCOVER") {
    return {
      ok: true, providerResourceId: rid,
      state: resourceState.state,
      capabilities: [{
        type: profile.capabilityType,
        advertised: profile.advertised,
      }],
      idempotent: true,
    };
  ...
  // RECONCILE: query provider state (no state change).
  if (action === "RECONCILE") {
    const state = await queryMockProviderState(profile.code, rid);
    return {
      ok: true, providerResourceId: rid,
      state: state.state,
      idempotent: true,
  ...
  // SETTLE: deferred — the mock declares it as supported but it's a no-op.
  if (action === "SETTLE") {
    return { ok: true, providerResourceId: rid, state: resourceState.state, idempotent: true };
  }
