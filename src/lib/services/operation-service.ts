...
// COMPARE-AND-SET fencing (audit issue #3):
// Atomically updates the session state ONLY if generation == expectedGen.
// Returns true if applied, false if stale (generation changed).
// This is a single SQL statement — no read-then-update race window.
...
// withTimeout: wraps a promise with a bounded timeout (audit issue #1).
// Never allows an adapter Promise to hang indefinitely.
// Returns { ok: false, error: "TIMEOUT" } if the timeout fires.
